...
- **Pending (final sign-off):** the positive UDS **round-trip** (`TestPositiveEndToEnd`) can only pass on Linux — runs on the new `docker-security` CI job. Optional follow-up: add an attack agent that tries to read a second run's input and asserts it cannot, for an explicit isolation assertion.
