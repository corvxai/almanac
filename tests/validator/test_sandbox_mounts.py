...
def test_volumes_bind_only_socket_and_own_input():
    vols = sd._sandbox_volumes("/host/run/almanac", ".almanac_stdin_RUN_A.json")

    # Exactly two file binds — never the whole socket dir.
    assert len(vols) == 2
    assert "/host/run/almanac" not in vols  # the dir itself is NOT mounted

    assert vols["/host/run/almanac/proxy.sock"] == {
        "bind": "/run/almanac/proxy.sock",
        "mode": "ro",
    }
    assert vols["/host/run/almanac/inputs/.almanac_stdin_RUN_A.json"] == {
        "bind": "/run/almanac/input.json",
        "mode": "ro",
    }
...
def test_volumes_are_per_run_isolated():
    ...
    a = sd._sandbox_volumes("/host/run/almanac", ".almanac_stdin_RUN_A.json")
    b = sd._sandbox_volumes("/host/run/almanac", ".almanac_stdin_RUN_B.json")
