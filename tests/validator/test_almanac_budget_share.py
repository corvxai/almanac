...
def test_score_almanac_applies_budget_share_to_epoch_budget(
    ...
    monkeypatch.setitem(
        sys.modules,
        "src.validator.almanac.metadata_manager",
        SimpleNamespace(MetadataManager=object),
    )
    almanac_loop = importlib.import_module("src.validator.almanac.loop")

    monkeypatch.setattr(almanac_loop, "fetch_trading_history", lambda **kw: [])
    monkeypatch.setattr(almanac_loop, "fetch_tao_price", lambda: 100.0)
    monkeypatch.setattr(almanac_loop, "compute_epoch_budget", lambda metagraph, tao_price_usd: 200.0)
    ...
    monkeypatch.setitem(sys.modules, "src.validator.almanac.scoring", fake_scoring_module)
    ...
    out = almanac_loop.score_almanac(
        network="finney",
        wallet=None,
